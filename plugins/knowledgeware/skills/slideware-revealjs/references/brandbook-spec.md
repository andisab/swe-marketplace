# Brandbook spec (slideware-revealjs)

A brandbook is a `.md` file with **YAML frontmatter** (design tokens) and a **prose body** (palette + type pairing + distinctive treatments). The same file format works for both `slideware-pptx` and `slideware-revealjs` — write once, render in either format.
...
The `slideware-pptx` renderer strips the `#` (pptxgenjs expects 6-char hex without). The `slideware-revealjs` renderer **keeps** the `#` (CSS expects it). One brandbook works for both — each renderer handles the format conversion.
