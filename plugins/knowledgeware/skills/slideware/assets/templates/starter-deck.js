...
//   node $HOME/.claude/skills/slideware/scripts/load-style.js <name|path> -o style-tokens.json
//   # Or for bundled brandbooks, copy the pre-built tokens (no parse needed):
//   cp $HOME/.claude/skills/slideware/assets/styles/custom/tokens/<name>.json style-tokens.json
//
...
if (!fs.existsSync(tokensPath)) {
  ...
  console.error("  node $HOME/.claude/skills/slideware/scripts/load-style.js <name|path> -o style-tokens.json");
  console.error("  # or copy a pre-built bundled one:");
  console.error("  cp $HOME/.claude/skills/slideware/assets/styles/custom/tokens/<name>.json style-tokens.json");
  process.exit(1);
}
...
async function buildDeck() {
  ...
  // See: $HOME/.claude/skills/slideware/references/layout-patterns.md
  ...
  console.log("  Polish:  python3 $HOME/.claude/skills/slideware/scripts/polish-deck.py '" + outFile + "'");
  console.log("  Render:  bash $HOME/.claude/skills/slideware/scripts/render-slides.sh '" + outFile + "' ./preview");
}
