...
Path convention: `$PLUGIN` = the knowledgeware plugin root (this format lives at `$PLUGIN/skills/slideware/pptx/`). Shared style tooling is at `$PLUGIN/scripts/`; slideware-specific scripts are at `$PLUGIN/skills/slideware/pptx/scripts/`.
...
cp $PLUGIN/skills/slideware/pptx/assets/templates/{starter-deck.js,package.json} .
npm install
# Author build-deck.js
node build-deck.js ./output
python3 $PLUGIN/skills/slideware/pptx/scripts/polish-deck.py ./output/deck.pptx
...
```bash
bash $PLUGIN/skills/slideware/pptx/scripts/render-slides.sh <sample.pptx> ./sample-preview/
...
```bash
mkdir -p my-deck/output && cd my-deck
cp $PLUGIN/skills/slideware/pptx/assets/templates/starter-deck.js ./build-deck.js
cp $PLUGIN/skills/slideware/pptx/assets/templates/package.json ./package.json
npm install   # ~10-15s
```
...
```bash
node $PLUGIN/skills/slideware/pptx/scripts/text-fit.js --text "..." --w 3 --h 1.2 --fontSize 14
```
...
```bash
bash $PLUGIN/skills/slideware/pptx/scripts/render-slides.sh ./output/deck.pptx ./output/preview/
```
...
2. `bash $PLUGIN/skills/slideware/pptx/scripts/render-slides.sh <existing.pptx> ./existing-preview/` to see the visuals.
